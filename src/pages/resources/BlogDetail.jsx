...
function BlogDetail() {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            ...
            setError(null);
            try {
                const data = await contentService.fetchBlogById(id);
                setBlog(data);
            } catch (err) {
                const message = err?.message || '';
                // If gated by backend with 403, direct to login
                if (message.toLowerCase().includes('403') || message.toLowerCase().includes('forbidden')) {
                    alert('Please login to continue reading.');
                    navigate('/login');
                } else if (message.toLowerCase().includes('not found')) {
                    setError('This blog was not found.');
                } else {
            ...
            <ContentWrapper>
                {loading ? (
                    ...
                    <ErrorState>
                        ...
                        <div>{error}</div>
                        <BackButton onClick={() => navigate('/resources/blogs')}>
                            ← Back to Blogs
                        </BackButton>
                    </ErrorState>
                ) : blog ? (
                    <>
                        ...
                        <Meta>
                            <MetaItem>
                                <span>👤</span>
                                <span>{blog.author_name || 'JUNGLORE'}</span>
                            </MetaItem>
                            ...
                            <FeaturedImage>
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        e.target.parentNode.innerHTML = '📝 Blog Image';
                                    }}
                                />
                            </FeaturedImage>
                        )}
